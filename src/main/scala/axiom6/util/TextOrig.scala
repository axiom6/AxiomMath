...
object TextOrig
{
  ...
  // Constructors
  def apply( cap:Int ) : TextOrig = new TextOrig(cap)
  def apply( cs:CS   ) : TextOrig = new TextOrig(cs)  // Works for TextOrig String etc
def apply( sq:CS*  ) : TextOrig =
{
  var len = 0
  for( s <- sq )
    len += s.length
  val text = new TextOrig(len)
  for( s <- sq )
    text.app(s)
  text
}
  ...
  val empty : TextOrig = new TextOrig(0)
  def is( text:TextOrig  ) : Boolean = text!=null && text != empty
  ...
  def equ( a:CS, b:CS )                 : Boolean = TextOrig.equ( a,  0, a.length, b, 0, b.length )
  def equ( a:CS, ab:Int, al:Int, b:CS ) : Boolean = TextOrig.equ( a, ab, al,       b, 0, b.length )
...
class TextOrig( _cap:Int ) extends TextOrig.CS
{
  ...
  */
  ...
  def app( t:TextOrig ) { if(t!=this) app( t.a ) }
  ...
  */
  ...
  def text( args:Any* ) : TextOrig = { seq(args); this }
  ...
  def fwd( cs:CS,_b:Int,_e:Int, df:Int ) : Int =
  {
    ...
    while( i < e )
    { if( TextOrig.equ(a,i,e,cs)) return i else i+=1 }
    df
  }
  ...
  def tab( n:Int )
  { lock(); try { var i = 1; while( i <= n ) { app(TextOrig.tab); i +=1 } } finally unlock() }
  ...
  */
  ...
  def text( tx:TextOrig ) : TextOrig = { tx.app(this); tx }
  ...
  def upper( i:Int ) { a(i) = TextOrig.toUpper(a(i)) }
  def lower( i:Int ) { a(i) = TextOrig.toLower(a(i)) }
